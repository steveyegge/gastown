...
while IFS= read -r PR_JSON; do
  ...
  while IFS= read -r CHECK; do
    ...
    FAILURES+=("$PR_NUM|$PR_TITLE|$CHECK_NAME")
...
for F in "${FAILURES[@]}"; do
  IFS='|' read -r PR_NUM PR_TITLE CHECK_NAME <<< "$F"
...
```bash
SUMMARY="$REPO: $PR_COUNT PRs — ${#EASY_WINS[@]} easy win(s), ${#NEEDS_REVIEW[@]} need review, ${#FAILURES[@]} failure(s), $CREATED bead(s) created"
echo "$SUMMARY"
```
