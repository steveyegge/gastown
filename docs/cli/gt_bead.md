---
title: "DOCS/CLI/GT BEAD"
---

## gt bead

Bead management utilities

### Synopsis

Utilities for managing beads across repositories.

Provides operations that span multiple beads repositories, such as
moving beads between repos and viewing beads by ID with automatic
prefix-based routing.

Subcommands:
  move    Move a bead from one repository to another
  show    Show details of a bead (routes by prefix)
  read    Alias for show

### Options

```
  -h, --help   help for bead
```

### SEE ALSO

* [gt](../cli/gt/)	 - Gas Town - Multi-agent workspace manager
* [gt bead move](../cli/gt_bead_move/)	 - Move a bead to a different repository
* [gt bead read](../cli/gt_bead_read/)	 - Show details of a bead (alias for 'show')
* [gt bead show](../cli/gt_bead_show/)	 - Show details of a bead

