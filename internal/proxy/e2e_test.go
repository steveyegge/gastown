...
func TestE2EEchoThroughProxy(t *testing.T) {
	...
	t.Run("echo hello world", func(t *testing.T) {
		...
		// Server injects --identity <rig>/<name> before the original args.
		assert.Equal(t, "--identity gastown/e2etest hello world\n", string(output))
	})
	...
	t.Run("echo no args", func(t *testing.T) {
		...
		output, err := cmd.CombinedOutput()
		require.NoError(t, err, "echo through proxy failed: %s", output)
		assert.Equal(t, "--identity gastown/e2etest\n", string(output))
	})
	...
	t.Run("echo multiple arguments preserves order", func(t *testing.T) {
		...
		output, err := cmd.CombinedOutput()
		require.NoError(t, err, "echo through proxy failed: %s", output)
		assert.Equal(t, "--identity gastown/e2etest a b c d\n", string(output))
	})
