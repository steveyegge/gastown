...
func TestIsToolPath(t *testing.T) {
	tests := []struct {
		...
		// .github/hooks/ paths — only gastown-specific files
		{".github/hooks/gastown.json", true},
		{".github/hooks/gastown-pretool-guard.sh", true},
		{".github/hooks/pre-commit", false},
		{".github/hooks/post-checkout", false},
		// Non-tool paths
		{"main.go", false},
...
func TestCleanExcludingToolFiles(t *testing.T) {
	tests := []struct {
		...
		{
			name: "only gastown hooks untracked",
			status: UncommittedWorkStatus{
				HasUncommittedChanges: true,
				UntrackedFiles:        []string{".github/hooks/gastown.json"},
			},
			want: true,
		},
		{
		...
		},
		{
			...
			status: UncommittedWorkStatus{
				HasUncommittedChanges: true,
				UntrackedFiles:        []string{".copilot/state", ".github/hooks/gastown-pretool-guard.sh"},
