...
// Implementation is platform-specific: flock(2) on Unix, in-process mutex on Windows.
// See flock_unix.go and flock_windows.go.
...
func (t *Tmux) sendMessageToTarget(target, text string) error {
	timeout := constants.NudgeReadyTimeout
