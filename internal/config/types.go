...
type MergeQueueConfig struct {
	...
	// BatchMerge enables batch mode where multiple PRs are merged together
	// before running tests once. This reduces CPU load when processing many PRs.
	BatchMerge bool `json:"batch_merge,omitempty"`

	// BatchSize is the maximum number of PRs to include in a single batch.
	// Only used when BatchMerge is enabled. Default: 5
	BatchSize int `json:"batch_size,omitempty"`

	// BatchWindow is the time window to wait for collecting PRs into a batch.
	...
	// "bisect-on-fail": Binary search to find failing PR(s) on failure.
