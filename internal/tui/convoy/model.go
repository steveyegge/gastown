...
func loadTrackedIssues(townBeads, convoyID string) ([]IssueItem, int, int) {
	...
	cmd := exec.CommandContext(ctx, "sqlite3", "-json", dbPath, query) // #nosec G204 -- sqlite3 with controlled args
	var stdout bytes.Buffer
	cmd.Stdout = &stdout
...
func getIssueDetailsBatch(townBeads string, issueIDs []string) map[string]IssueItem {
	...
	cmd := exec.CommandContext(ctx, "bd", args...) // #nosec G204 -- bd is a trusted internal tool
	cmd.Dir = townBeads
	var stdout bytes.Buffer
	cmd.Stdout = &stdout
