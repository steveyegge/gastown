...
//
// Why delete-only (no recreation)? Claude Code only reads settings at startup -
// it doesn't hot-reload them during a session (see anthropics/claude-code#5513).
// Recreating files would give a false sense of security since running agents
// wouldn't pick up the changes anyway. We delete + warn, letting agent startup
// create correct settings when they restart. This keeps creation logic in one place.
