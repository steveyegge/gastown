...
// primeContent is the Gas Town PRIME.md content — compact fallback when gt prime hook fails.
// Role templates provide full context; this covers only essentials.
const primeContent = `# Gas Town Worker Context

Recovery: ` + "`gt prime`" + ` for full context. Hook has work → RUN IT (no waiting).
Commands: ` + "`gt hook`" + `, ` + "`gt mail inbox`" + `, ` + "`bd ready`" + `, ` + "`bd show <id>`" + `, ` + "`bd close <id>`" + `

## Session Close

1. ` + "`git status && git add <files> && git commit -m \"...\" && git push`" + `
2. ` + "`gt done`" + ` (polecats: submits to merge queue and exits)
`
