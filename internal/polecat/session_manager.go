...
func (m *SessionManager) Start(polecat string, opts SessionStartOptions) error {
	...
	if err := lifecycle.EnsureSettingsForRole(polecatSettingsDir, workDir, "polecat", runtimeConfig); err != nil {
		return fmt.Errorf("ensuring runtime settings: %w", err)
	}

	// Build startup command with beacon for predecessor discovery.
	// The startup bootstrap helper handles no-prompt/no-hook fallback delivery.
