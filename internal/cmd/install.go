...
// town root. Claude Code rebases its CWD to the git root (~/gt/), so
// role-specific CLAUDE.md files in subdirectories (mayor/, deacon/) are not
// loaded. This file provides a baseline identity anchor that survives compaction.
//
...
//
// Returns (created bool, error) - created is false if the file already exists.
