...
// TestCheckCrossRigGuard_RejectsCrossRig verifies the guard rejects cross-rig beads.
// With per-rig beads (sbx-gastown-vrb4), the guard always runs (no flat namespace bypass).
func TestCheckCrossRigGuard_RejectsCrossRig(t *testing.T) {
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beads")
	if err := os.MkdirAll(beadsDir, 0755); err != nil {
		t.Fatal(err)
...
`
	...
	// Cross-rig should be rejected
	err := checkCrossRigGuard("bd-ka761", "gastown/polecats/Toast", tmpDir)
	if err == nil {
		t.Error("checkCrossRigGuard should reject cross-rig bead")
	}
}
