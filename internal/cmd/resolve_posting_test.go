...
// TestAgentEnv_PostingAllRoles_WorkerPostings is the counterpart to
// TestAgentEnv_PostingAllRoles but uses WorkerPostings config (persistent
// posting) instead of .runtime/posting (session posting). It verifies the
// end-to-end flow: resolvePostingName picks up the persistent config,
// resolvePostingLevel determines the template level, and AgentEnv sets
// GT_POSTING_LEVEL correctly.
func TestAgentEnv_PostingAllRoles_WorkerPostings(t *testing.T) {
	t.Parallel()

	roles := []struct {
		role       string
		agentName  string
		subdirKind string // "polecats" or "crew"
	}{
		{"polecat", "Toast", "polecats"},
		{"crew", "alice", "crew"},
	}
	...
	for _, r := range roles {
		t.Run(r.role, func(t *testing.T) {
			...
			// Set up rig config with persistent posting (WorkerPostings)
			rigPath := filepath.Join(townRoot, rigName)
			settingsDir := filepath.Join(rigPath, "settings")
			if err := os.MkdirAll(settingsDir, 0755); err != nil {
				t.Fatal(err)
			...
			settings := config.RigSettings{
				Type:           "rig-settings",
				Version:        1,
				WorkerPostings: map[string]string{r.agentName: "dispatcher"},
			}
			data, _ := json.Marshal(settings)
			if err := os.WriteFile(filepath.Join(settingsDir, "config.json"), data, 0644); err != nil {
				t.Fatal(err)
			...
			// Work dir with NO session posting (.runtime/posting)
			workDir := filepath.Join(rigPath, r.subdirKind, r.agentName)
			if err := os.MkdirAll(workDir, 0755); err != nil {
				t.Fatal(err)
			}

			// Phase 1: resolvePostingName should return the config posting
			ctx := RoleInfo{
				Role:     Role(r.role),
				Rig:      rigName,
				Polecat:  r.agentName,
			...
			// Phase 2: resolvePostingLevel determines template resolution level
			ctx.Posting = name
			level, _ := resolvePostingLevel(ctx)
			if level != "embedded" {
				t.Fatalf("resolvePostingLevel() = %q, want %q", level, "embedded")
			}

			// Phase 3: AgentEnv should propagate both GT_POSTING and GT_POSTING_LEVEL
			env := config.AgentEnv(config.AgentEnvConfig{
				Role:         r.role,
				Rig:          rigName,
				AgentName:    r.agentName,
