...
func TestGetAgentIdentity_PolecatAmbiguousPosting(t *testing.T) {
	ctx := RoleContext{
		Role:             RolePolecat,
		Rig:              "gastown",
		Polecat:          "furiosa",
	...
	got := getAgentIdentity(ctx)
	want := "gastown/polecats/furiosa[rig:inspector]"
	if got != want {
		t.Errorf("getAgentIdentity() = %q, want %q", got, want)
	}
}

func TestGetAgentIdentity_CrewAmbiguousPosting(t *testing.T) {
	ctx := RoleContext{
		Role:             RoleCrew,
		Rig:              "gastown",
		Polecat:          "diesel",
	...
	got := getAgentIdentity(ctx)
	want := "gastown/crew/diesel[town:scout]"
	if got != want {
		t.Errorf("getAgentIdentity() = %q, want %q", got, want)
	}
}

func TestGetAgentIdentity_PolecatNotAmbiguousPosting(t *testing.T) {
	// When not ambiguous, level should NOT appear even if PostingLevel is set
	ctx := RoleContext{
		Role:             RolePolecat,
		Rig:              "gastown",
		Polecat:          "furiosa",
		Posting:          "inspector",
		PostingLevel:     "rig",
		PostingAmbiguous: false,
	}
	got := getAgentIdentity(ctx)
	want := "gastown/polecats/furiosa[inspector]"
	if got != want {
		t.Errorf("getAgentIdentity() = %q, want %q", got, want)
	}
}
