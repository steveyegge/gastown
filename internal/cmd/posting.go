...
var postingShowCmd = &cobra.Command{
	...
	Long: `Show the current posting assignment or display a posting template.

With no arguments, displays the session-level and persistent posting
state for this agent.

With a posting name argument, displays the posting template content
(resolved through rig, town, and built-in layers).

Examples:
  gt posting show              # Show current posting state
  gt posting show dispatcher   # Display dispatcher posting content
  gt posting show scout        # Display scout posting content`,
...
func runPostingShow(cmd *cobra.Command, args []string) error {
	// If a posting name is given, display its template content.
	if len(args) > 0 {
		return showPostingContent(args[0])
	}

	// No argument: show current session/persistent posting state.
...
// showPostingContent loads and displays a posting template by name.
...
func runPostingDrop(cmd *cobra.Command, args []string) error {
	...
	if current == "" {
		persistent, workerName := getPersistentPosting()
		if persistent != "" {
			fmt.Printf("No session posting to drop (persistent posting %q exists for %s; use: gt crew post %s --clear)\n",
				persistent, workerName, workerName)
		} else {
