...
func outputCrewContext(ctx RoleContext) {
	...
	fmt.Println("- `bd close <issue>` - Mark issue complete")
	fmt.Println()
	fmt.Println("## Crew Communication")
	fmt.Println("Reliable messaging via adapter REST (verify-and-retry enabled):")
	fmt.Println("```")
	fmt.Println("# Send message to a crew member")
	fmt.Println("curl -s -X POST http://localhost:8080/api/agents/{name}/prompt \\")
	fmt.Println("  -H 'Content-Type: application/json' -d '{\"prompt\": \"your message\"}'")
	fmt.Println("")
	fmt.Println("# Check crew status")
	fmt.Println("curl -s http://localhost:8080/api/crew/status")
	fmt.Println("```")
	fmt.Println("Agent names: st-crew-dunks, ta-crew-timmy, cc-crew-cc, cm-crew-charlie, nv-crew-neil, gtk-crew-tooly")
	fmt.Println()
	fmt.Println("**Note:** `" + cli.Name() + " nudge --mode=immediate` is being updated (st-emo) to route")
	fmt.Println("through the adapter REST API automatically, with raw tmux send-keys as fallback.")
	fmt.Println()
	fmt.Println("## Hookable Mail")
	fmt.Println("Mail can be hooked for ad-hoc instructions: `" + cli.Name() + " hook attach <mail-id>`")
	fmt.Println("If mail is on your hook, read and execute its instructions (GUPP applies).")
