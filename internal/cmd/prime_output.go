...
func outputPostingContext(ctx RoleContext, data templates.RoleData) {
	...
	result, err := templates.LoadPosting(ctx.TownRoot, rigPath, ctx.Posting)
	if err != nil {
		// Design §7: unconditional stderr warning for undefined postings.
		// Visible to the human operator, not injected into agent priming.
		fmt.Fprintf(os.Stderr, "warning: posting %q not defined at any level\n", ctx.Posting)
		fmt.Fprintf(os.Stderr, "  Define it at:\n")
		if rigPath != "" {
			fmt.Fprintf(os.Stderr, "    Rig:  %s/postings/%s.md.tmpl\n", rigPath, ctx.Posting)
		}
		if ctx.TownRoot != "" {
			fmt.Fprintf(os.Stderr, "    Town: %s/postings/%s.md.tmpl\n", ctx.TownRoot, ctx.Posting)
		}
